...
# Failure notification queue
failure_queue = None
sending_failure_count_threshold = 10
...
def _sendVideoData():
    global video_destination
    global stream_id
    global failure_queue
    global sending_failure_count_threshold
    sending_failure_count = 0
    ...
    while True:
        try:
            ...
            print(e)
            sending_failure_count += 1
            print(f"_sendVideoData failed {sending_failure_count} time(s).")
            if sending_failure_count > sending_failure_count_threshold:
                print("_sendVideoData failure threshold exceeded. Initiating graceful exit.")
                failure_queue.put(e)
...
def _pipelineFunc():
    ...
    try:
        ...
        print(e)
        print("_pipelineFunc failed. Initiating graceful exit.")
        failure_queue.put(e)
...
def _encodingFunc():
    ...
    while True:
        try:
            ...
            print(e)
            print("_encodingFunc failed. Initiating graceful exit.")
            failure_queue.put(e)
...
def _sendGPSData():
    ...
    try:
        ...
        print(e)
        print("_sendGPSData failed. Initiating graceful exit.")
        failure_queue.put(e)
...
def start_jobs():
    ...
    while True:
        ...
        if failure_queue.qsize() > 0:
            print("One of the child processes failed. Terminating all processes.")
            terminate_jobs()
            break
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-e",
        "--enable_gps",
        help="Enable (optional) sending of GPS information",
        default=False,
        type=bool
    )
    parser.add_argument(
